...
class Tweet extends React.Component {
  ...
  componentDidMount() {
    const { currentUser, router, tweet } = this.props;
    this.setState({
  ...
  tweetDropdown = () => {
    const{currentUser, tweet} = this.props;
    var menu1 = (
  ...
  render() {
    const { router, tweet, currentUser } = this.props;
    ...
    return(
      ...
      >
        <div className="tweet__wrapper">
          ...
          <div
            style={{marginLeft:10}}
            className="tweet__box tweet__footer"
          >
            {this.footerIcons()}
          </div>
        </div>
